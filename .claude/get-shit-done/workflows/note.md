...
- **Global scope**: `/Users/elias/Claude Code/Portfolio/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/elias/Claude Code/Portfolio/.claude/notes/`)
...
2. Glob `/Users/elias/Claude Code/Portfolio/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/elias/Claude Code/Portfolio/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/elias/Claude Code/Portfolio/.claude/notes/` — works in any directory
